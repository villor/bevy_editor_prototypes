...
impl DynamicScene {
    ...
    {
        let construct = |context: &mut ConstructContext,
                         type_id: TypeId,
                         patches: &Vec<Box<dyn DynamicComponentPatch>>| {
        ...
        match self.component_props.entry(TypeId::of::<C>()) {
            Entry::Vacant(entry) => {
                entry.insert(ComponentProps {
                    ...
                    construct: DynamicConstructFn::Typed(construct),
                });
            }
            Entry::Occupied(mut entry) => {
                let entry = entry.get_mut();
                if matches!(entry.construct, DynamicConstructFn::Reflected) {
                    entry.construct = DynamicConstructFn::Typed(construct);
                }
    ...
    pub fn construct_components(
        ...
        for (type_id, component_props) in self.component_props.iter() {
            let ComponentProps { construct, patches } = component_props;
            match construct {
                DynamicConstructFn::Typed(construct) => {
                    (construct)(context, *type_id, patches)?;
                }
                DynamicConstructFn::Reflected => {
                    construct_reflected_component(context, *type_id, patches)?;
                }
            };
        }
        Ok(())
